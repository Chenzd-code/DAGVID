...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        if df is not None:
            self.df = df
            if hasattr(self,'user_num'):
                self._compute_user_interaction_count()
            return
        # if all files exists
    ...
    def _compute_user_interaction_count(self):
        """计算每个用户的交互次数"""
        if self.df is None or len(self.df) == 0:
            # 如果数据框为空，创建全零张量
            self.user_interaction_count = torch.zeros(self.user_num, dtype=torch.int32)
            return

        # 使用pandas的value_counts统计每个用户的交互次数
        user_counts = self.df[self.uid_field].value_counts()
        ...
        for user_id, count in user_counts.items():
            if user_id < self.user_num:  # 确保用户ID在有效范围内
    ...
    def split(self):
        ...
        # 只对训练集计算用户交互次数
        full_ds[0]._compute_user_interaction_count()

        # 验证集和测试集不需要计算用户交互次数
        # 但为了确保它们有该属性，可以设置为None或空张量
        for i in range(1, 3):
            if not hasattr(full_ds[i], 'user_interaction_count'):
                full_ds[i].user_interaction_count = None
    ...
    def copy(self, new_df):
        ...
        # # 复制用户交互次数属性
        if hasattr(self, 'user_interaction_count'):
            nxt.user_interaction_count = self.user_interaction_count.clone()
        return nxt
