...
def build_sim(context):
    ...
    context_norm = torch.where(torch.isnan(context_norm), torch.zeros_like(context_norm), context_norm)
...
#         # 将稀疏张量转换为稠密张量以进行topk操作
#         adj_dense = adj.to_dense()
#         knn_val, knn_ind = torch.topk(adj_dense, topk, dim=-1)
#
#         # 创建稀疏索引和值
#         row_indices = torch.arange(adj.shape[0], device=device).unsqueeze(1).expand(-1, topk)
#         indices = torch.stack([row_indices.flatten(), knn_ind.flatten()])
#         values = knn_val.flatten()
#
#         # 创建新的稀疏张量
#         sparse_adj = torch.sparse_coo_tensor(indices, values, adj.shape, device=device)
#
#         # 获取拉普拉斯矩阵
#         edge_index, edge_weight = get_sparse_laplacian(
#             sparse_adj.coalesce().indices(),
#             sparse_adj.coalesce().values(),
